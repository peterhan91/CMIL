...
def main(args):
    ...
    ############# Create the CT dataset using NPZ_Dataset #############
    ...
    if global_rank == 0 and args.log_dir is not None and not args.eval:
        ...
        log_writer = wandb.init(project='Merlin_finetune', dir=args.log_dir, config=args)
    else:
        log_writer = None
    ...
    if args.finetune and not args.eval:
        enc = build.ImageEncoder()
        checkpoint = torch.load(args.finetune, map_location='cpu')
        new_state_dict = {}
        for key in checkpoint.keys():
            new_key = key.replace("encode_image.", "")  # Remove the "encode_image." prefix
            new_state_dict[new_key] = checkpoint[key]

        msg = enc.load_state_dict(new_state_dict, strict=False)
        print(msg)
