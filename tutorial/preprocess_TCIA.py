...
def process_image(row_dict, save_path, new_shape=(384, 512, 512)):
    ...
    try:
        ...
        file_path = os.path.join('/mnt/nas/CT/NSCLC_Radiogenomics_NII', row['VolumeName']+ '.nii.gz')
    ...
    # Save as nii.gz file
    img_path = os.path.join(save_path, f"{img_name}.npz")
...
def convert_to_nii(annotations_csv, save_path, new_shape=(384, 512, 512)):
    ...
    processed = [os.path.basename(x).replace('npz', '') for x in list(glob(os.path.join(save_path, '*.npz')))]
...
if __name__ == '__main__':
    # Example usage
    annotations_file = '/home/than/DeepLearning/CMIL/csvs/NSCLC_Radiogenomics_metadata.csv'  # CSV file with image-text annotations
    os.makedirs('/mnt/nas/CT/npz_npy_nsclc_radiogenomics', exist_ok=True)
    output_path = '/mnt/nas/CT/npz_npy_nsclc_radiogenomics'  # Output path for the NIfTI dataset
