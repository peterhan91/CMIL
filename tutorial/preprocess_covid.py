...
def process_image(row_dict, save_path, new_shape=(384, 512, 512)):
    ...
    nii, img_data = read_nii_data(file_path)
    slope = nii.header['scl_slope']
    intercept = nii.header['scl_inter']
    ...
    img_data = img_data[:, ::-1, :] # Flip the image along the y-axis
...
if __name__ == '__main__':
    # Example usage
    annotations_file = '../csvs/inspect2.csv'  # CSV file with image-text annotations
    os.makedirs('/mnt/nas/CT/npz_npy_INSPECT', exist_ok=True)
    output_path = '/mnt/nas/CT/npz_npy_INSPECT'  # Output path for the NIfTI dataset
